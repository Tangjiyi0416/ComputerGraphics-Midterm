...
// For grid quad
Quad* g_pRQuad;				// 位於畫面正中間
Quad* g_pGQuad[GRID_SIZE];	// 為 Red Quad 的 child
Quad* g_pBQuad[GRID_SIZE];	// x為 Green Quad 的 child
float  g_fRQuadT[3];
...
void AutoMotion(float delta) {
	g_step += delta;
	mat4 mxGR, mxBR;
	g_fGAngle = 90.0f * g_step;
	g_fBAngle = -90.0f * g_step;
...
int main(int argc, char** argv)
{
	...
	glutMainLoop();
	if (g_pRQuad != nullptr)delete g_pRQuad;
