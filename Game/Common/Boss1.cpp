	...
	:Enemy(parent, localPosition, localRotation, localScale)
{
	...
	_children.pushBack(new Boss2(this, vec3(-0.7071f, -0.5f, 0)));
	_collider = new Collider(static_cast<GameObject*>(this), ColliderType::Enemy, vec2(localPosition.x, localPosition.y), vec2(1, 1), vec2(localScale.x * 1.f, localScale.y * 1.f), std::bind(&Boss1::Onhit, this, std::placeholders::_1), 0b1);
}
